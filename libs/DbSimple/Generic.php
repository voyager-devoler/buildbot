...
class DbSimple_Generic
{
    ...
    function parseDSN($dsn)
    {
        ...
        if (isset($parsed['host']) && $parsed['host'] == 'socket')
        {
            $sock = substr($parsed['path'],0,strpos($parsed['path'],'>'));
            $parsed['path'] = substr($parsed['path'], strpos($parsed['path'],'>')+1);
            $parsed['host'] = ':'.$sock;
        }
        $parsed['dsn'] = $dsn;
        return $parsed;
